...
#
# Timing: the poll interval is 1s, so capture tests sleep ~2s to be safe. The
# whole script takes ~10s.
#
...
# Wipe the zclip dir WITHOUT recreating it; daemon must refuse to auto-create.
rm -rf "$ZDIR"
...
sleep 2 # poll interval is 1s; give it a full tick + margin
...
start_daemon "$LOG"; pid=$DAEMON_PID
sleep 1
# Foreground second daemon: it should hit the pidfile flock and bail fast.
...
start_daemon "$LOG"; pid=$DAEMON_PID
sleep 1
# `use` writes content tagged dev.zclip.origin; daemon must NOT re-capture it.
"$ZCLIP" use 5 >/dev/null 2>&1
sleep 2
# Exactly one row with this content (the seeded one) — no daemon-inserted dup.
